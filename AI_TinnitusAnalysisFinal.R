...
setwd("C:/Users/18133/OneDrive - The University of Texas-Rio Grande Valley/Dr.Vinaya/Project1_Tinitus/Paper 2")
#setwd("D:/Onedrive_H/OneDrive - The University of Texas-Rio Grande Valley/Dr.Vinaya/Project1_Tinitus/Paper 2")


load("AI_TinnitusAnaRevised2021.RData")
load("AI_TinnitusAnaRevised2021_hpComp.RData")
...
#Source Missing Data: https://cran.r-project.org/web/packages/naniar/vignettes/naniar-visualisation.html
...
TIDataRevisedAna2$TFIReductionCat[TIDataRevisedAna2$TFIReduction_New>=13]<-"1"  # "Sucsess"
...
t.test(TIDataRevisedAna2$PRE_TFIScore,TIDataRevisedAna2$NewPost_TFIScore2, paired=TRUE)
shapiro.test(TIDataRevisedAna2$TFIReduction_New)#Since p-value = 0.4145, normality assumption is satisfied.

#table(TIDataRevisedAna2$GroupNew,TIDataRevisedAna2$TFIReductionCat)
...
#############Analysis started on 30/12/2020
#This contains the all the variables that we consider for Paper 1 (not the best selected model)
# modelnew5 <- glm(TFIReductionCat~Multi_Sounds+Gender+Loud._Noise_Exposure+
#                    Work_Less+Hearing_Loss+Psychological_Condition+Past_Treatment+HearingAid_usage+Sounds.can.distract+Medication_for_Tinnitus+
#                    Tinnitus_Location+TypeTinn_Ringing+TypeTinn_Buzzing+TypeTinn_HighPitch+TypeTinn_LowPitch+TypeTinn_Pulsing+TypeTinn_Clicking+TypeTinn_Music+
#                    TypeTinn_Voices+TypeTinn_Humming+Education_Level+Employment_Type+Cat_Age+Cat_TinnDuration+Anxiety_c+Depression_c+Insomnia_c+Hyperacusis_c+HHIA_c+CFQ_c+SWLS_c+How_of_Tinnitus_Heard,
#                  data =training14, family = binomial) %>%stepAIC(trace = FALSE)
# summary(modelnew5)
...
#To get help
?rpart()
#rparttreeRevised.modelnew is the revised model ran based on all dicotaomus variables (all clinical factors and several other) considered in AI paper 1
...
rparttreeRevised.modelnew<- rpart(TFIReductionCat~Cat_Age+Gender+Education_Level+Employment_Type+
                                    ...
                                    Anxiety_c+Depression_c+Insomnia_c+Hyperacusis_c+HHIA_c+CFQ_c+SWLS_c,training14,parms=list(split="gini"),
                               control = (minsplit = 20,minbucket = round(20/3), xval = 3, maxdepth = 10))
rparttreeRevised.modelnewrpart.control
summary(rparttreeRevised.modelnew)

rparttreeRevised.modelnew$variable.importance
...
# The proportion of customers that were predicted to take
# the offer out of those who actually took the offer is 0.9640666

# The proportion of customers that were predicted not to take
# the offer out of those who actually did not take the offer
# is 0.249379
12
n <- sum(conf.matrix)
...
accuracy <- sum(diag) / n
accuracy
# The percentage of all correct predictions that were made
# by the model 82.02%
...
mod_ADA <- Predictor$new(adabo, data = ZX, type = "prob")
# Then we explain the first instance of the dataset with the Shapley() method:
shapley_ADA <- Shapley$new(mod_ADA, x.interest=ZX[1,])
shapley_ADA$results
plot(shapley_ADA)


XR <- Boston[which(names(Boston) != "medv")]
predictor <- Predictor$new(rf, data = XR, y = Boston$medv)

shapley <- Shapley$new(predictor, x.interest = XR[1, ])
shapley <- Shapley$new(mod, x.interest = XR[1, ])
shapley$plot()
...
shapley <- Shapley$new(predictor, x.interest = ZX1[1, ])
shapley$plot()
...
save.image("AI_TinnitusAnaRevised2021_hpComp.RData")
